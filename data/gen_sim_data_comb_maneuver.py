...
for i in trange(M): #M = len(timeGNSS)
    ...
    if timeGNSS[i] <= tmc:
        z_GNSS[i, 0] = 10 + np.random.normal(0, 0.1)
        z_GNSS[i, 1] = 2*timeGNSS[i] + np.random.normal(0, 0.1)
        z_GNSS[i, 2] = 1 + np.random.normal(0, 0.4)
    ...
    if timeGNSS[i] > tmc:
        ...
        z_GNSS[i, 2] = (1/z_ampl * r * omega * np.cos((omega / z_ampl ) * timeGNSS[i-tmc])) + np.random.normal(0, 0.4)
...
#     scipy.io.savemat(mat_file, {"z_GNSS": z_GNSS.T})
    ...
    # %%
with open("simulation_params_comb_maneuver_long_ver3.mat","w+") as mat_file:
    scipy.io.savemat(mat_file, {'leverarm': leverarm.T})
    scipy.io.savemat(mat_file, {'S_a': S_a.T})
    scipy.io.savemat(mat_file, {'S_g': S_g.T})
